...
GOOS=windows GOARCH=amd64 go build -o deej.exe -ldflags "-H=windowsgui -s -w -X main.gitCommit=$GIT_COMMIT -X main.versionTag=$VERSION_TAG -X main.buildType=$BUILD_TYPE" ./pkg/deej/cmd
if [ $? -eq 0 ]; then
    echo 'Done.'
else
